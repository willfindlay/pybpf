...
class ProjectInit:
    ...
    def __init__(
        ...
        self.bpf_dir = os.path.join(self.project_dir, bpf_dir)
    ...
    def bootstrap_project(self, overwrite = False):
        ...
        for depth, (root, dirs, files) in enumerate(os.walk(TEMPLATES_DIR)):
            for _file in files:
                ...
                if depth:
                    subdirs = '/'.join(root.split(os.sep)[-depth:])
                else:
                    subdirs = ''
    ...
    @drop_privileges
    def compile_bpf_skeleton(self, bpf_src=None, cflags=[]) -> str:
        """
        ...
        skel_so = os.path.join(self.project_dir, os.path.basename(strip_full_extension(obj_file) + '.skel.so'))
